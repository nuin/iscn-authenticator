...
- An API key from your administrator. Keys look like `iscn_live_3f7a…e9c1`.
- A way to make HTTPS requests (curl, Postman, `fetch`, `requests`, etc.).

If you don't have a key yet, contact support or — for local development — mint one yourself:
```bash
cd deno
...
Default: **60 requests per minute per key**. Every response carries `X-RateLimit-Remaining` and `X-RateLimit-Reset`. Over-quota returns `429` with `Retry-After` (seconds):
...
|---|---|---|---|
| `unauthenticated` | 401 | Missing/wrong/revoked key | Check header; request a new key |
| `rate_limited` | 429 | Over quota | Back off `Retry-After` seconds |
