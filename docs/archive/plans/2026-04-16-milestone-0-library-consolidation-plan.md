...
├── deno/
│   ├── lib/
│   │   └── validator.ts              (kept - app-specific native validator wrapper)
...
Note: `deno/lib/validator.ts` stays. It contains the Deno-specific native validator wrapper, which is application code, not library code.
...
- `deno/` — Deno Deploy web app. Imports `packages/core/` directly; contains an app-specific native validator wrapper in `deno/lib/validator.ts`.
